import RHRouteC.Littlewood.OmegaLowerBound
import RHRouteC.Ingham.ZeroRepulsion
import Mathlib.NumberTheory.LSeries.RiemannZeta
import Mathlib.Analysis.SpecialFunctions.Log.Basic
import Mathlib.Analysis.SpecialFunctions.Exp

namespace RHRouteC.RouteC
...
theorem exp_loglog_dominates_sq (C c₁ : ℝ) (hC : 0 < C) (hc₁ : 0 < c₁) :
  ...
  have hcore : Tendsto (fun v : ℝ => c₁ * Real.exp v / v - 2 * v) atTop atTop := by
    refine hmul.congr' ?_; filter_upwards [eventually_gt_atTop (0 : ℝ)] with v hv; have hv' : v ≠ 0 := ne_of_gt hv; field_simp; ring
...
theorem riemannHypothesis_of_growth_and_repulsion (hG : GrowthBound) (hR : ZeroRepulsion) : _root_.RiemannHypothesis := by
  intro s hs htriv hs1; by_contra hre
  ...
  exact absurd (lt_of_le_of_lt (le_trans hge (hub t h2)) (hTa t hTat)) (lt_irrefl _)
...
/-- **Bridge from Littlewood (unconditional falsification of GrowthBound).** -/
theorem RH_from_littlewood (hL : Littlewood.LittlewoodOmegaLowerBound_OPEN) : ¬GrowthBound := by
  intro hG; obtain ⟨c, hc, hOm⟩ := hL
  obtain ⟨C, hC, hB⟩ := hG
  obtain ⟨Ta, hTa⟩ := eventually_atTop.mp (exp_loglog_dominates_sq C c hC hc)
